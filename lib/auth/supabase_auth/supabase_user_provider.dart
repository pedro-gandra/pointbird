...
class PointbirdSupabaseUser extends BaseAuthUser {
  PointbirdSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> pointbirdSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = PointbirdSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
